"""
Secure-channel primitives for Hospital-MP (Flask).

Implements the same AES-256-GCM envelope as the Rust (chat-orch),
TypeScript (agent-runtime), Go (conversation-chat), Python (Compliance),
and Java (UN_email_send_ms) services. Wire format:

    { "v": 1, "iv": <b64 12B>, "ct": <b64>, "tag": <b64 16B> }

Detected via Content-Type: application/vnd.unagent.secure+json +
X-Secure-Channel: aes256gcm/1. Configure once at startup via init_from_env().
"""
...
_state: dict[str, Any] = {"key": None, "aesgcm": None, "enabled": False}


def init_from_env() -> None:
    """Configure the channel from process env. Call once at startup."""
...
def open_envelope(env: dict[str, Any]) -> bytes:
    """Verify and decrypt the envelope. Raises ChannelError on tamper / bad key."""
    if _state["aesgcm"] is None:
        raise ChannelError("channel not initialised")
    if env.get("v") != 1:
    ...
    except Exception as e:  # InvalidTag and its kin
        raise ChannelError(f"AEAD verification failed: {e}") from e


def seal_json(value: Any) -> Tuple[bytes, str, bool]:
    """Marshal value to JSON and seal when active. Returns (body, content_type, encrypted)."""
...
def open_bytes(content_type: Optional[str], data: bytes) -> bytes:
    """Decrypt when content_type marks the bytes as an envelope; passthrough otherwise."""
